...
Component({
  ...
  observers: {
    'chartData.**': function (val) {
      if (typeof val === 'object') {
        if (val.series && val.series.length > 0) {
          ...
          const mixinDatacomLoading = true;
          this._clearChart();
          const showchart = false;
          const mixinDatacomErrorMessage = null;
          this.setData({ mixinDatacomLoading, showchart, mixinDatacomErrorMessage });
        }
      } else {
        const mixinDatacomLoading = false;
        const showchart = false;
        const mixinDatacomErrorMessage = '参数错误：chartData数据类型错误';
        this.setData({ mixinDatacomLoading, showchart, mixinDatacomErrorMessage });
      }
    },
    localdata(val) {
    ...
    'opts.**': function (val) {
      if (typeof val === 'object') {
        ...
        const mixinDatacomLoading = false;
        const showchart = false;
        const mixinDatacomErrorMessage = '参数错误：opts数据类型错误';
        this.setData({ mixinDatacomLoading, showchart, mixinDatacomErrorMessage });
      }
    },
    reshow(val) {
    ...
    errorMessage(val) {
      if (val && this.data.errorShow && val !== null && val !== 'null' && val !== '') {
        const mixinDatacomLoading = false;
        const showchart = false;
        const mixinDatacomErrorMessage = val;
        this.setData({ mixinDatacomLoading, showchart, mixinDatacomErrorMessage });
      } else {
        const showchart = false;
        const mixinDatacomErrorMessage = null;
  ...
  methods: {
    beforeInit() {
      ...
      if (
        typeof this.data.chartData === 'object' &&
        this.data.chartData != null &&
        this.data.chartData.series !== undefined &&
        this.data.chartData.series.length > 0
      ) {
        // 拷贝一下chartData，为了opts变更后统一数据来源
        const _drawData = deepCloneAssign({}, this.data.chartData);
        const mixinDatacomLoading = false;
    ...
    localdataInit(resdata) {
      ...
      if (needCategories === true) {
        ...
        if (
          ...
          this.data.chartData.categories.length > 0
        ) {
      ...
      // 拷贝一下chartData，为了opts变更后统一数据来源
      const _drawData = deepCloneAssign({}, tmpData);
    ...
    checkData(anyData) {
      ...
      if (this.data.type && cfu.type.includes(this.data.type)) {
        ...
        const mixinDatacomLoading = false;
        const showchart = false;
        const mixinDatacomErrorMessage = '参数错误：props参数中type类型不正确';
        this.setData({ mixinDatacomLoading, showchart, mixinDatacomErrorMessage });
      }
      // 挂载categories和series
    ...
    resizeHandler() {
      ...
      const lastDrawTime = this.data._lastDrawTime ? this.data._lastDrawTime : currTime - 3000;
      ...
      const chartdom = wx
        ...
        .boundingClientRect((data) => {
          if (data.width > 0 && data.height > 0) {
            const mixinDatacomLoading = false;
            const showchart = true;
            const _lastDrawTime = Date.now();
            const cWidth = data.width;
            const cHeight = data.height;
            const mixinDatacomErrorMessage = null;
            this.setData({
              mixinDatacomLoading,
              showchart,
            ...
            cfu.option[cid].background =
              this.data.background == 'rgba(0,0,0,0)' ? '#FFFFFF' : this.data.background;
              ...
              query
                ...
                .exec((res) => {
                  if (res[0]) {
                    ...
                    this.data.mixinDatacomErrorMessage =
    ...
    _touchStart(e) {
      ...
      this.emitMsg({
        name: 'getTouchStart',
        params: { type: 'touchStart', event: e.changedTouches, id: cid },
      });
    },
    _touchMove(e) {
      const cid = this.data.cid;
      ...
      if (
        this.data.ontap === true &&
        cfu.option[cid].enableScroll === false &&
        this.data.onmovetip === true
      ) {
        this._tap(e, true);
      }
      if (
      ...
      this.emitMsg({
        name: 'getTouchMove',
        params: { type: 'touchMove', event: e.changedTouches, id: cid },
      });
    },
    _touchEnd(e) {
      const cid = this.data.cid;
      ...
      this.emitMsg({
        name: 'getTouchEnd',
        params: { type: 'touchEnd', event: e.changedTouches, id: cid },
      });
      if (
        this.data.ontap === true &&
        cfu.option[cid].enableScroll === false &&
